...
<head>
    ...
    <style>
        ...
        .status-container { margin-top: 10px; background: #0f172a; padding: 12px 25px; border-radius: 15px; border: 1px solid rgba(16, 185, 129, 0.4); box-shadow: 0 20px 50px rgba(0,0,0,0.8); }
        
        /* --- Sharp Alignment Lockdown --- */
        .observation-deck > *:not(.hud-overlay):not(.erp-metrics):not(.latency-engine):not(.hologram-stage):not(.ai-wing):not(.trend-panel):not(.control-console):not(#envHeader):not(.top-nav):not(#userBadge) {
