...
<head>
    ...
    <style>
        ...
        .latency-engine { position: absolute; right: 40px; top: 250px; display: flex; flex-direction: column; gap: 20px; z-index: 100; }
        .latency-card { 
    ...
    <script>
        ...
        async function syncCommandCenter() {
            try {
                ...
                const latHistory = await smartFetch(['./allure/widgets/latency-trend.json']).catch(() => []);
                const latestLat = (latHistory && latHistory.length > 0) ? latHistory[0] : null;
                
                const avgDuration = total > 0 ? (summary?.time?.duration / total) : 0;
                
                // Use REAL tactical metrics if available, fallback to weighted baseline
                const apiLatency = latestLat ? latestLat.apiLatency : avgDuration * 0.4;
                const uiLatency = latestLat ? latestLat.uiLatency : avgDuration * 0.6; 
