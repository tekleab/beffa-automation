...
<head>
    ...
    <style>
        ...
        .latency-engine { position: absolute; right: 40px; top: 280px; display: flex; flex-direction: column; gap: 15px; z-index: 100; }
        .latency-card { 
    ...
    <script>
        ...
        async function syncCommandCenter() {
            try {
                ...
                const latHistory = await smartFetch([
                ...
                const suiteAvg = total > 0 ? (summary?.time?.duration / total) : 0;
                
                // Use REAL tactical metrics (ms), fallback to a normalized weighted baseline (clipped to 10s max for safety)
                const apiLatency = latestLat ? latestLat.apiLatency : Math.min(suiteAvg * 0.4, 10000);
                const uiLatency = latestLat ? latestLat.uiLatency : Math.min(suiteAvg * 0.6, 10000); 
                ...
                const updatePerfCard = (idPrefix, val) => {
                    ...
                    animateValue(idPrefix + 'Value', 0, val, 1000, 'ms');
