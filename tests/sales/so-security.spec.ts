...
test.describe('Security & Temporal Isolation Audits @sales @security @regression @full', () => {
    ...
    test('Guardrail: System must strictly segregate invoices and receipts by Customer', async ({ page , request }) => {
        const app = new AppManager(page);
    ...
    test('Guardrail: System must explicitly reject historical back-dated invoices', async ({ page , request }) => {
        const app = new AppManager(page);
        const meta = sharedMeta;
        const item = sharedItem;
