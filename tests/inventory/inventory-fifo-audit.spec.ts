 ...
 * 
 * Objective: Verify that the ERP correctly manages cost layers across 
...
test.describe('FIFO Costing Forensic Audit@inventory @security @costing @regression @full', () => {
    ...
    test('Audit: 7-Stage FIFO Layer Validation & COGS Accuracy', async ({ page }) => {
        ...
        console.log(`[STAGE 1] Beginning Balance: ${stage1.currentStock} @ $${stage1.unitCost}`);
        expect(stage1.currentStock).toBe(10);
        expect(stage1.unitCost).toBe(100);

        // 🟦 STAGE 2: Adding Layer 1 (20 units @ $200)
        console.log(`[FIFO] Stage 2: Adding Layer 1 (20 units @ $200)...`);
        ...
        // 🟨 STAGE 3: Adding Layer 2 (15 units @ $300)
        console.log(`[FIFO] Stage 3: Adding Layer 2 (15 units @ $300)...`);
        ...
        // 🟩 STAGE 4: Invoice 1 — The Deep Consumption (20 units)
        console.log(`[FIFO] Stage 4: Draining Layer 1 and 10 units of Layer 2...`);
        ...
        // 🔍 FORENSIC AUDIT: Check Journals for COGS accuracy
        const journals = await app.api.inventory.getJournalEntriesAPI(inv1.id);
        
        // Find the Inventory credit entry (Asset reduction) or COGS debit (Expense)
        ...
        console.log(`[AUDIT REPORT] Invoice #: ${inv1.ref}`);
        console.log(`[AUDIT REPORT] Item    : ${item.itemName} (${item.id})`);
        ...
        // FINAL ASSERTION
        expect(actualCostImpact).toBeCloseTo(expectedCogs, 2);
        ...
        // 🟥 STAGE 5: Adjustment 1 — The Smart Loss (-5 units)
        console.log(`[FIFO] Stage 5: Verifying Cost of Loss (-5 units)...`);
        ...
        console.log(`[FIFO] Stage 6: Revaluing inventory (DNA Modification)...`);
        ...
        console.log(`[STAGE 6] Master Cost immediately revalued to: $${stage6.unitCost} (Expect: $190)`);
        expect(stage6.unitCost).toBe(190);

        // 💀 STAGE 7: Invoice 2 — The Final Boss (15 units)
        console.log(`[FIFO] Stage 7: The Final Boss Sale (15 units)...`);
        ...
        console.log(`[FINAL] Audit Complete. Stock: ${finalState?.currentStock} | Final Master Cost: $${finalState?.unitCost}`);
        expect(finalState?.currentStock).toBe(5);
        expect(finalState?.unitCost).toBe(290); // (The remaining 5 units were from the $300 layer, revalued to $290)
    });
});
