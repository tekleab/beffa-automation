 ...
 */
test.describe('Project Management: Lifecycle & CRUD Audits @project @audit @regression @full', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        console.log(`[SETUP] Customer: ${meta.customerName} (${meta.customerId})`);
        await page.close();
    });
    ...
    test('API-01: Create project returns valid id, ref and pending status', async () => {
        ...
        console.log(`[OK] Created: ${projectRef} (${projectId})`);
    });

    test('API-02: GET single project returns correct fields', async () => {
        const d = await app.api.project.getProjectAPI(projectId);
        expect(d.id).toBe(projectId);
        expect(d.project_name).toBe(projectName);
    ...
    test('API-03: remaining_balance = estimated_revenue - estimated_expense', async () => {
        const d = await app.api.project.getProjectAPI(projectId);
    ...
    test('API-04: LIST projects includes newly created project', async () => {
        const projects = await app.api.project.listProjectsAPI();
        expect(projects.find((p: any) => p.id === projectId)).toBeTruthy();
    });

    test('API-05: LIST filter by customer_id returns only matching projects', async () => {
    ...
    test('API-06: LIST filter project_status=pending includes new project', async () => {
        const projects = await app.api.project.listProjectsAPI({ project_status: 'pending' });
        expect(projects.find((p: any) => p.id === projectId)).toBeTruthy();
    });

    test('API-07: LIST search by name fragment finds project', async () => {
        const fragment = projectName.split('-').slice(0,2).join('-');
    ...
    test('API-08: PATCH project_name persists', async () => {
        projectName = `${projectName}-UPD`;
        await app.api.project.updateProjectAPI(projectId, { project_name: projectName });
        const d = await app.api.project.getProjectAPI(projectId);
        expect(d.project_name).toBe(projectName);
    });

    test('API-09: PATCH estimated_revenue recalculates remaining_balance', async () => {
        await app.api.project.updateProjectAPI(projectId, { estimated_revenue: 300000 });
        const d = await app.api.project.getProjectAPI(projectId);
        expect(parseFloat(d.remaining_balance)).toBe(300000 - 80000);
    });

    test('API-10: PATCH estimated_expense recalculates remaining_balance', async () => {
        await app.api.project.updateProjectAPI(projectId, { estimated_expense: 100000 });
        const d = await app.api.project.getProjectAPI(projectId);
        expect(parseFloat(d.remaining_balance)).toBe(300000 - 100000);
    });

    test('API-11: PATCH percent_complete persists', async () => {
        await app.api.project.updateProjectAPI(projectId, { percent_complete: 25 });
        const d = await app.api.project.getProjectAPI(projectId);
    ...
    test('API-12: Status pending → in-progress is accepted', async () => {
        const p = await app.api.project.setProjectStatusAPI(projectId, 'in-progress');
        expect(p.status).toBe('in-progress');
    });

    test('API-13: Status in-progress → completed is accepted', async () => {
        const p = await app.api.project.setProjectStatusAPI(projectId, 'completed');
        expect(p.status).toBe('completed');
    });

    test('API-14: Status completed → pending is accepted (reopen)', async () => {
        const p = await app.api.project.setProjectStatusAPI(projectId, 'pending');
        expect(p.status).toBe('pending');
    });

    // ── API: EDGE CASES ────────────────────────────────────────────────────────

    test('API-15: completion_method=task_completion accepted, percent stays 0 without tasks', async () => {
        await app.api.project.updateProjectAPI(projectId, { completion_method: 'task_completion' });
        const d = await app.api.project.getProjectAPI(projectId);
        expect(d.completion_method).toBe('task_completion');
        expect(parseFloat(d.percent_complete)).toBe(0);
        await app.api.project.updateProjectAPI(projectId, { completion_method: 'manual' });
    });

    test('API-16: expense > revenue yields negative remaining_balance (no server guard)', async () => {
        ...
        await app.api.project.updateProjectAPI(projectId, { estimated_revenue: 300000, estimated_expense: 100000 });
    });

    // ── UI: LIST PAGE ──────────────────────────────────────────────────────────

    test('UI-01: Projects list page loads with all 9 table columns', async ({ page }) => {
        ...
        // From screenshot: Project Name, Workspace, Workflow, Customer, Start Date, End Date, Status, Progress, Budget, Tasks
    ...
    test('UI-02: Filter pills (Workspace, Workflow, Status, Budget) are visible', async ({ page }) => {
        ...
        await page.goto('/project-management/projects');
        await page.waitForLoadState('networkidle');
        // From screenshot — filter pills in toolbar
    ...
    test('UI-08: Project created via API appears in the list', async ({ page }) => {
        ...
        await page.goto('/project-management/projects');
        await page.waitForLoadState('networkidle');
        await expect(page.getByText(projectRef, { exact: false }).first()).toBeVisible({ timeout: 15000 });
    });
    ...
    test('UI-09: Clicking project row navigates to detail view', async ({ page }) => {
        ...
        await page.goto('/project-management/projects');
        await page.waitForLoadState('networkidle');
        await page.getByText(projectRef, { exact: false }).first().click();
        await page.waitForLoadState('networkidle');
        expect(page.url()).toContain('project');
        await expect(page.getByText(projectRef, { exact: false }).first()).toBeVisible({ timeout: 10000 });
    });
    ...
    test('UI-11: Add Project form has project name input', async ({ page }) => {
        ...
        await page.getByRole('button', { name: /Add Project/i }).click();
        await page.waitForTimeout(2000);
        const container = page.locator('[role="dialog"]').first().isVisible({ timeout: 3000 }).then(v => v
        ...
        if (!visible) {
            // Fallback: any text input in form is acceptable
    ...
    test('UI-12: Project detail page shows financial data (budget / revenue / balance)', async ({ page }) => {
        const a = new AppManager(page);
        await a.login(process.env.BEFFA_USER, process.env.BEFFA_PASS);
        await page.goto(`/project-management/projects/${projectId}`);
        await page.waitForLoadState('networkidle');
