...
test.describe('Project Management: UI List Page @project @smoke', () => {
    ...
    async function setup(page: any) {
        // Intercept /api/projects requests: strip empty query params (like project_status=) so backend doesn't filter on empty strings
    ...
    async function gotoProjectsList(page: any) {
        await page.goto('/project-management/projects');
        // Wait for SPA workspace loading screen to finish and table/toolbar elements to render
    ...
    test('UI-02: Filter pills (Workspace, Workflow, Status, Budget) are visible', async ({ page }) => {
        ...
        for (const pill of ['Workspace', 'Workflow', 'Status', 'Budget']) {
            await expect(page.getByRole('button', { name: new RegExp(pill, 'i') }).first()).toBeVisible({ timeout: 10000 });
        }
    });
    ...
    test('UI-03: Sort and View buttons are visible in toolbar', async ({ page }) => {
        const { app } = await setup(page);
        await gotoProjectsList(page);
        await expect(page.getByRole('button', { name: /Sort/i }).first()).toBeVisible({ timeout: 10000 });
    ...
    test('UI-04: Advanced filters and Command filters links are present', async ({ page }) => {
        ...
        await expect(page.getByText(/Command filters/i).first()).toBeVisible({ timeout: 10000 });
    });
    ...
    test('UI-05: Add Project button and Export button are visible', async ({ page }) => {
        ...
        await expect(addProjectEl).toBeVisible({ timeout: 10000 });
    ...
    test('UI-07: Pagination shows rows-per-page and page count', async ({ page }) => {
        const { app } = await setup(page);
        await gotoProjectsList(page);
        await expect(page.getByText(/Rows per page/i).first()).toBeVisible({ timeout: 10000 });
        await expect(page.getByText(/Page [0-9]+ of [0-9]+/i).first()).toBeVisible({ timeout: 10000 });
    });
