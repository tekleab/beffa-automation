...
test.describe('Purchase Order - Standalone Workflow', () => {
    ...
    test('Create Purchase Order and Verify Lifecycle', async ({ page }) => {
        ...
        await billVendorBtn.waitFor({ state: 'visible', timeout: 30000 });
        await billVendorBtn.click({ timeout: 5000 });
        await page.waitForTimeout(1000);
        await page.locator('[role="option"], [role="menuitem"], .chakra-menu__menuitem')
                  .filter({ hasText: selectedVendor }).first()
                  .click({ force: true });
        ...
        // Item Receipt logic
        console.log("Action: Receiving items from Purchase Order...");
        const receivedPoTab = page.getByRole('tab', { name: /Received Purchase Order/i });
        await receivedPoTab.waitFor({ state: 'visible', timeout: 5000 });
        await receivedPoTab.click();
        await page.waitForTimeout(2000);

        // Click the first checkbox to receive the PO line item we created
        const itemCheckbox = page.locator('.chakra-checkbox__control').first();
        if (await itemCheckbox.isVisible()) {
            await itemCheckbox.click({ force: true });
            await page.waitForTimeout(1000);
            console.log("Info: Items successfully received.");
        } else {
            console.log("Warning: No PO items found to receive!");
        }
