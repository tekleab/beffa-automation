...
export class InventoryPage {
  ...
  async _extractItemDetails(itemName: string): Promise<{ itemName: string; itemId: string | null; currentStock: number; unitCost: number; salesAccountCode: string; costAccountCode: string; inventoryAccountCode: string }> {
    ...
    const clean = (f: string) => f.match(/^\d+/)?.[0] || '';
  ...
  async createInventoryAdjustmentUI(itemName: string, adjQty: number = 1): Promise<{ ref: string; id: string }> {
    ...
    const adjUUIDMatch = this.page.url().match(/\/adjustments\/([a-f0-9-]+)/);
    const adjUUID = adjUUIDMatch ? adjUUIDMatch[1] : '';
