...
- Name: inventory/inv-load.spec.ts >> Load: Concurrent Inventory Adjustments @inventory @full >> LOAD: 10 concurrent +5 adjustments must produce exactly +50 net stock
...
```ts
  ...
  14  | 
  15  | 
  16  | test.describe('Load: Concurrent Inventory Adjustments @inventory @full', () => {
  ...
  34  |             cost_method_code: 'FIFO', quantity: 10, unit_cost: 100
  35  |         });
...
> 64  |         expect(finalStock, `Lost update under concurrent advance: expected ${expectedStock}, got ${finalStock}`).toBe(expectedStock);
  ...
  70  |             cost_method_code: 'FIFO', quantity: 20, unit_cost: 100
  71  |         });
