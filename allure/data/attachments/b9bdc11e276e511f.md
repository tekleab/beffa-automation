...
- Name: inventory/inventory-average-audit.spec.ts >> Weighted Average Costing Forensic Audit @inventory @security @costing @regression @full >> Audit: 7-Stage Average Cost Validation & COGS Accuracy
...
Received difference:   66.66999999999999
```
...
```ts
  ...
  9   |  */
  10  | 
  11  | test.describe('Weighted Average Costing Forensic Audit @inventory @security @costing @regression @full', () => {
...
> 62  |         expect(stage2?.unitCost).toBeCloseTo(166.67, 1);
  ...
  162 |             customerId: envMeta.customerId,
```