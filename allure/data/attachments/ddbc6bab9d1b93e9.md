...
- Name: purchase/procurement-document-integrity.spec.ts >> Procurement Document Integrity Attacks @purchase @security @logic @regression @full >> Guardrail: Concurrent identical PO submissions must not create duplicate liability
- Location: tests/purchase/procurement-document-integrity.spec.ts:118:9

# Error details

```
Error: [CRITICAL_LOGIC_BUG] Duplicate PO race succeeded — both POs approved and billed. Total AP liability doubled to 0. No deduplication guard exists.
```
...
```ts
  ...
  146 |             const totalLiability = parseFloat(b1Data.total ?? b1Data.amount ?? 0) + parseFloat(b2Data.total ?? b2Data.amount ?? 0);
  147 | 
