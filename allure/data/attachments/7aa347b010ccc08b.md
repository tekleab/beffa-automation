...
- Location: tests/sales/so-load.spec.ts:32:9

# Error details

```
Error: More than 1 invoice failed — possible backend overload
...
```ts
  ...
  5   | /**
  6   |  * =============================================================================
  7   |  * MODULE: Sales Order - Load & Performance Suite
  8   |  * ARCHITECTURAL SCOPE & COVERAGE:
  9   |  * 1. Bulk SO list endpoint responds within threshold under load
  10  |  * 2. SO detail page latency verified under concurrent reads
  11  |  * 3. Pagination performance with large dataset
  12  |  * =============================================================================
  13  |  */
  14  | 
  15  | 
  ...
  35  |             cost_method_code: 'FIFO', quantity: 100, unit_cost: 100
  36  |         });
  37  | 
...
> 66  |         expect(failed.length, 'More than 1 invoice failed — possible backend overload').toBeLessThanOrEqual(1);
  ...
  74  |             cost_method_code: 'FIFO', quantity: 50, unit_cost: 100
  75  |         });
  76  | 
