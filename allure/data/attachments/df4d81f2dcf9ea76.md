...
- Location: tests/cross-module/line-item-miscellaneous-audit.spec.ts:415:9
...
Call log:
  ...
  - waiting for locator('.chakra-modal__content, .chakra-popover__content, [role="dialog"]').filter({ hasText: /Warehouse \*|G\/L Account \*|Description/i }).first()
    19 × locator resolved to <section tabindex="-1" role="dialog" id="popover-content-:rds:" aria-describedby="popover-body-:rds:" class="chakra-popover__content css-1rnzimv">…</section>
                  ...
                  - generic [ref=e205]:
                    ...
                    - generic [ref=e219]:
                      - group [ref=e220]:
                        ...
                        - textbox "Invoice number" [disabled] [ref=e223]: INV/2026/08/13/001745
      ...
      - generic [ref=e313]:
        - generic [ref=e314]:
          - group [ref=e316]:
            ...
            - button "Item selector" [ref=e318]: steam door 60g 60x202
          - group [ref=e320]:
            - generic [ref=e321]: Warehouse *
            - button "Warehouse selector" [ref=e322]: Default Warehouse
          - group [ref=e324]:
            - generic [ref=e325]: Location *
            - button "Location selector" [ref=e326]: Default Warehouse Location
          - group [ref=e328]:
            - generic [ref=e329]: Quantity *
            - spinbutton [ref=e331]: "800"
        - generic [ref=e332]:
          - group [ref=e334]:
...
```ts
  ...
  170 |             const lbl = modal.locator('label').filter({ hasText: labelName }).first();
  171 |             if (await lbl.isVisible({ timeout: 1000 }).catch(() => false)) {
  172 |                 const container = lbl.locator('..').first(); // Parent element
  173 |                 const inp = container.locator('input:not([disabled])').first();
  174 |                 if (await inp.isVisible({ timeout: 1000 }).catch(() => false)) {
  175 |                     await inp.fill(opts.unitPrice);
  176 |                     priceFilled = true;
  177 |                     console.log(`[MODAL] Filled price field via label "${labelName.source}": ${opts.unitPrice}`);
  178 |                     break;
  179 |                 }
  180 |             }
  181 |         }
...
> 214 |         await expect(modal).not.toBeVisible({ timeout: 15000 });
  ...
  232 |         await app.selectRandomOption(page.locator('.flex-col, .chakra-form-control').filter({ hasText: /Account.?Receivable/i }).locator('button').first(), 'Accounts Receivable');
  ...
  257 |         await app.selectRandomOption(page.locator('.flex-col, .chakra-form-control').filter({ hasText: /Account.?Receivable/i }).locator('button').first(), 'Accounts Receivable');
  ...
  288 |         await app.selectRandomOption(page.locator('.flex-col, .chakra-form-control').filter({ hasText: /Account.?Receivable/i }).locator('button').first(), 'Accounts Receivable');
