...
Call log:
  - waiting for locator('#customer_name-input-id, input[name="name"], input[placeholder*="name" i], input[id*="name" i]').first() to be visible
    43 × locator resolved to hidden <input value="" type="text" placeholder="Search by name" class="chakra-input css-1jlznti"/>
                  ...
                  - list [ref=f1e177]:
                    ...
                    - listitem [ref=f1e184]:
                      - link "Detail" [ref=f1e185] [cursor=pointer]:
                        - /url: /receivables/customers/d6b9aaea-4c8d-4c59-8c88-a6086a9220a4/detail
...
```ts
  ...
  73  |         // Edit may navigate to /edit URL or render inline — wait for either
  74  |         await page.waitForLoadState('domcontentloaded', { timeout: 15000 }).catch(() => {});
  75  |         // Broaden selector: edit form may use different input ID than create form
  76  |         const nameInput = page.locator('#customer_name-input-id, input[name="name"], input[placeholder*="name" i], input[id*="name" i]').first();
> 77  |         await nameInput.waitFor({ state: 'visible', timeout: 20000 });
      |                         ^ TimeoutError: locator.waitFor: Timeout 20000ms exceeded.
  78  |         await expect(nameInput).toBeEditable({ timeout: 15000 });
  79  |         await nameInput.clear();
  80  |         await nameInput.fill(updatedName);
  81  |         const saveBtn = page.locator('button:has-text("Save"), button:has-text("Update")').first();
  82  |         await saveBtn.waitFor({ state: 'visible', timeout: 10000 });
  83  |         await saveBtn.click({ force: true });
  84  |         await page.waitForLoadState('networkidle', { timeout: 20000 }).catch(() => {});
  85  |         // Poll for updated name — React re-render may lag behind networkidle
  86  |         await expect(page.getByText(updatedName).first()).toBeVisible({ timeout: 30000 });
  87  |         console.log('[OK] Customer updated');
  88  | 
  89  |         // ── Phase 4: Remove ───────────────────────────────────────────────────
  90  |         console.log('[STEP] Phase 4: Removing customer');
  91  |         await page.waitForTimeout(2000);
  92  |         const removeBtn = page.getByRole('button', { name: /remove|delete/i }).first();
  93  |         await removeBtn.waitFor({ state: 'visible', timeout: 20000 });
  94  |         await removeBtn.scrollIntoViewIfNeeded().catch(() => {});
  95  |         await removeBtn.click();
  96  | 
  97  |         const confirmBtn = page.locator('section[role="dialog"] button, div[role="alertdialog"] button, [role="dialog"] button, .modal button').filter({ hasText: /Yes|Confirm|Delete|Remove/i }).first();
  98  | 
  99  |         if (!await confirmBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
  100 |             await removeBtn.click({ force: true });
  101 |         }
  102 | 
  103 |         await confirmBtn.waitFor({ state: 'visible', timeout: 15000 });
  104 |         await confirmBtn.click({ force: true });
  105 |         await page.waitForURL(url => url.href.includes('/receivables/customers'), { timeout: 30000 });
  106 |         console.log('[RESULT] Customer Lifecycle: PASSED');
  107 |     });
  108 | });
  109 | 
```