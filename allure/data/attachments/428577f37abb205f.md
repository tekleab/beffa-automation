...
```
Error: [CRITICAL_LOGIC_BUG] Bill BILL/2026/08/04/000338: Overpayment of 6000 on 3000 bill created negative balance=-3000. Vendor credit injection possible.
...
```ts
  ...
  58  |         const balance = parseFloat(billData.unpaid_amount ?? billData.balance ?? billData.amount_due ?? 0);
...
> 61  |         expect.soft(balance, `[CRITICAL_LOGIC_BUG] Bill ${bill.ref}: Overpayment of ${overpayAmount} on ${billAmount} bill created negative balance=${balance}. Vendor credit injection possible.`).toBeGreaterThanOrEqual(0);
  ...
  112 |         const balance = parseFloat(billData.unpaid_amount ?? billData.balance ?? billData.amount_due ?? 0);
  ...
  151 |         const billAmount = parseFloat(billData.unpaid_amount ?? billData.net_due ?? billData.total ?? 3000);
