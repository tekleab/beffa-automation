...
```
Error: [CRITICAL_LOGIC_BUG] Ghost payment PAY/2026/08/04/000031 accepted on fully-paid bill BILL/2026/08/04/000340! Vendor credit manipulation possible.
...
> 121 |             expect.soft(false, `[CRITICAL_LOGIC_BUG] Ghost payment ${ghost.ref} accepted on fully-paid bill ${bill.ref}! Vendor credit manipulation possible.`).toBe(true);
  ...
  202 |             const balance = parseFloat(billData.unpaid_amount ?? billData.balance ?? billData.amount_due ?? -1);
  ...
  213 |         const finalBalance = parseFloat(finalBill.unpaid_amount ?? finalBill.balance ?? finalBill.amount_due ?? -1);
