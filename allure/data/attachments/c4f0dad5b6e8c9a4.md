...
```ts
  ...
  65  |                 throw new Error(`[CRITICAL_PERIOD_CONTROL_BUG] System approved back-dated PO from previous fiscal year!`);
  66  |             } catch (advanceErr: any) {
  67  |                 if (advanceErr.message.includes('CRITICAL_PERIOD_CONTROL_BUG')) throw advanceErr;
...
> 94  |                 throw new Error(`[CRITICAL_PERIOD_CONTROL_BUG] System approved PO from next fiscal year!`);
      |                       ^ Error: [CRITICAL_PERIOD_CONTROL_BUG] System approved PO from next fiscal year!
  95  |             } catch (advanceErr: any) {
  96  |                 if (advanceErr.message.includes('CRITICAL_PERIOD_CONTROL_BUG')) throw advanceErr;
